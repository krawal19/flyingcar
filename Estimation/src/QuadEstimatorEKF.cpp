...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F attitude_ = attitude.Rotate_BtoI(accel) + V3F(0, 0, -9.81f); /*The acceleration (accel) is without gravity*/

  predictedState(0) += ekfState(3) * dt;
  predictedState(1) += ekfState(4) * dt;
  predictedState(2) += ekfState(5) * dt;
  predictedState(3) += attitude_.x * dt;
  predictedState(4) += attitude_.y * dt;
  predictedState(5) += attitude_.z * dt;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0, 1) = -c_phi * s_theta * s_psi + s_phi * c_psi;
